...
typedef STileView<UTileStyle*> SGridMapTileSetTileView;
...
private:
	TSharedRef<SWidget> GetAddTileStylePicker();
	TSharedRef<SWidget> BuildPaletteView();

	TSharedRef<ITableRow> GenerateTile(UTileStyle* Item, const TSharedRef<STableViewBase>& OwnerTable);
	void OnSelectionChanged(UTileStyle* Item, ESelectInfo::Type SelectInfo);
	...
	void AddTileStyle(const struct FAssetData& AssetData);
	void OnSearchTextChanged(const FText& InFilterText);

private:
	TSharedPtr<class SPositiveActionButton> AddTileSetCombo;
	TSharedPtr<class SSearchBox> SearchBoxPtr;
	TSharedPtr<class FAssetThumbnailPool> ThumbnailPool;
	TSharedPtr<SGridMapTileSetTileView> TileViewWidget;

	TArray<UTileStyle*> FilteredItems;
